...
Mate는 대학생 러닝 크루를 위한 커뮤니티 웹 애플리케이션입니다.  
...
<details>
<summary><b>Backend: ModuleNotFoundError</b></summary>

가상환경이 활성화되어 있는지 확인하세요 (`(venv)` 표시). 활성화 후 `pip install -r requirements.txt`를 다시 실행합니다.
</details>
...
1. 백엔드가 실행 중인지 확인
2. `backend/.env`에 `CORS_ORIGINS`에 `http://localhost:3000`이 포함되어 있는지 확인
3. 500 에러가 동반된 경우, CORS 자체가 아닌 백엔드 내부 오류가 원인 — 서버 로그 확인
</details>

<details>
<summary><b>Database: Password Authentication Failed</b></summary>

`.env`의 `DATABASE_URL`에 오타가 없는지 확인하세요. 비밀번호에 특수문자가 포함된 경우 URL 인코딩이 필요합니다.
</details>

---

<div align="center">
<sub>Built with ❤️ by Mate Team</sub>
</div>
